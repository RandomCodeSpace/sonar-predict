...
rem sonar-predictor launcher (Windows) — self-contained agent skill entry point.
rem
...
rem skill bundle is shape-complete; daemon-backed analysis on Windows is not
rem yet verified.
...
if not exist "%CLI_JAR%" (
    echo sonar-predictor: missing %CLI_JAR% - the skill bundle is incomplete. 1>&2
    exit /b 2
)
